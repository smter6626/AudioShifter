...
- `v0.1.0-alpha.3` 已完成项目所有者审核和非开发 Apple Silicon Mac 人工验收；下一步是在不重新构建、不移动 tag、不替换附件的前提下，将 Draft ID `364860803` 公开为 Pre-release，并在发布后核对公开 URL、三个附件的名称/大小/SHA-256 与现有审核记录一致。alpha.1 和 alpha.2 内部 Draft 保持不变，是否进入 Developer ID 签名与 Apple 公证阶段仍另行决定。
